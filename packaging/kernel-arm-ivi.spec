...
### this upstream_version *MUST* match one from file include/config/kernel.release
### In our case it is "3.14.19-4-arm-ivi"
%define upstream_version 3.14.19
%define variant arm-ivi
%define kernel_version %{version}-%{release}
%define kernel_full_version %{version}-%{release}-%{variant}
%define kernel_arch arm
%define kernel_arch_subdir arch/arm

Name: kernel-%{variant}
Summary: The Linux kernel
Group: System/Kernel
License: GPL-2.0
URL: https://gerrit.globallogic.com.ua/tizen/profile/ivi/kernel-arm-ivi
...
#    "have choice for virtual-setup-ivi-bootloader needed by kernel-arm-ivi: setup-extlinux setup-gummiboot"
...
make -s ARCH=%{kernel_arch} ivi_omap_defconfig
make -s ARCH=%{kernel_arch} %{?_smp_mflags}
...
cat %{kernel_arch_subdir}/boot/zImage %{kernel_arch_subdir}/boot/dts/dra7-evm-domU.dtb \
        > %{kernel_arch_subdir}/boot/vmlinuz-%{kernel_full_version}
install -m 755 %{kernel_arch_subdir}/boot/vmlinuz-%{kernel_full_version} \
	%{buildroot}/boot/vmlinuz-%{kernel_full_version}
...
rm -rf %{buildroot}/lib/modules/%{kernel_full_version}/build
rm -rf %{buildroot}/lib/modules/%{kernel_full_version}/source
