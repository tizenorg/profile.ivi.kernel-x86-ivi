...
static int ioh_irq_type(struct irq_data *d, unsigned int type)
{
	...
	unsigned long flags;
	u32 val;
	struct ioh_gpio *chip = irq_data_get_irq_chip_data(d);
	int irq = d->irq;
...
static struct irq_chip ml_ioh_irqchip = {
	.name		= "GPIO",
	.irq_enable	= ioh_irq_enable,
	.irq_disable	= ioh_irq_disable,
	.irq_mask	= ioh_irq_mask,
	.irq_unmask	= ioh_irq_unmask,
	.irq_set_type	= ioh_irq_type,
};
				...
				unsigned int irq_start, unsigned int num)
{
	...
	chip->irq_base = irq_start;
	for (i = 0; i < num; i++) {
		irq_set_chip_and_handler_name(chip->irq_base + i,
			&ml_ioh_irqchip, handle_simple_irq, "mux");
